...
if (isset($_POST["submit"])) {
  ...
  $sql = "UPDATE `data1` SET `studentname`='$studentname',`bookname`='$bookname',`startdate`='$startdate', `enddate`='$enddate' WHERE id = $id";
...
<body>
  ...
  <div class="container">
    ...
    <?php
    $sql = "SELECT * FROM `data1` WHERE id = $id LIMIT 1";
    $result = mysqli_query($conn, $sql);
        ...
        <div class="mb-3">
               <div class="mb-3">
                  ...
                  <div class="input-group">
                     <input type="date" class="form-control" id="datepicker" name="startdate" placeholder="Select a date" value="<?php echo $row['startdate'] ?>">
                  </div>
               </div>
            </div>
            <div class="mb-3">
               <div class="mb-3">
                  ...
                  <div class="input-group">
                     <input type="date" class="form-control" id="datepicker" name="enddate" placeholder="Select a date" value="<?php echo $row['enddate'] ?>">
                  </div>
               </div>
            </div>
        ...
        function populateDropdown(selectId, options) {
            ...
            for (const id in options) {
                if (options.hasOwnProperty(id)) {
                    ...
                    optionElement.textContent = options[id]; 
